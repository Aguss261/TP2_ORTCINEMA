...
class ProductoController {
    ...
    createProducto = async (req, res, next) => {
        try {
            ...
            res
                .status(200)
                .send({ success: true, message: "Producto creado con éxito" });
    ...
    getAllProductos = async (req, res, next) => {
        try {
            ...
            res
                .status(200)
                .send({ success: true, message: "Productos encontrados", result });
        } catch (error) {
            next(error);
        }
    };
    ...
    getProductoById = async (req, res, next) => {
        try {
            ...
            if (!result) throw new Error("No se encontró el producto");
            res
                .status(200)
                .send({ success: true, message: "Producto encontrado", result });
        } catch (error) {
            res.status(400).send({ success: false, message: error.message });
        }
    };
